...
STATE_ENTER(turn_left) {
	...
	hal_motor_turn_left(MOTOR_SPEED_FULL);
}
...
STATE_ENTER(turn_right) {
	...
	hal_motor_turn_right(MOTOR_SPEED_FULL);
}
...
void setup_fsm_movement() {
  ...
  // Slow down when backward obstacle is near
	...
	TIMED_TRANSITION(movement, turn_right, stop, 1000);
	TIMED_TRANSITION(movement, turn_left, stop, 1000);

	// Start reasoning when idle for too long
	TIMED_TRANSITION(movement, idle, reasoning, 10000);
